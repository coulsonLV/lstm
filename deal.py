...
def model_train(x1,x2,x3,x4,x5,x6,x7,y1,y2,y3,y4,y5,y6,y7):
    ...
    bde = decoder2(bde)
    rbde = decoder2(rbde)
    egode = TimeDistributed(Dense(2))(egode)
    lfde = TimeDistributed(Dense(2))(lfde)
    fde = TimeDistributed(Dense(2))(fde)    
    rfde = TimeDistributed(Dense(2))(rfde)    
    lbde = TimeDistributed(Dense(2))(lbde)    
    bde = TimeDistributed(Dense(2))(bde)    
    rbde = TimeDistributed(Dense(2))(rbde)
...
speed = np.hstack((egovx_out,egovy_out,lfvx_out,lfvy_out,fvx_out,fvy_out,rfvx_out,rfvy_out,lbvx_out,lbvy_out,bvx_out,bvy_out,rbvx_out,rbvy_out))
speed = speed.reshape(-1,700)
speed = np.hstack((speed[:,126:140],speed[:,266:280],speed[:,406:420],speed[:,546:560],speed[:,686:700]))
data1 = np.hstack((data,speed))

train_numx = int(len(data1)*0.8)
datasetx = data1[:train_numx]
testsetx = data1[train_numx:]
test_numx = len(testsetx)
trainx1 , trainy1 = datasetx[:,:-70],datasetx[:,-70:]
testx1 , testy1 = testsetx[:,:-70],testsetx[:,-70:]
trainx1 = trainx1.reshape((trainx1.shape[0],10,28))
testx1 = testx1.reshape((testx1.shape[0],10,28))
trainy1 = trainy1.reshape((trainy1.shape[0],5,14))
testy1 = testy1.reshape((testy1.shape[0],5,14))
...
trainpredictx  = np.array(trainpredictx1).reshape(-1,10)
prev = trainpredictx[:15044,:]
prevlf = trainpredictx[15044:30088,:]
prevf = trainpredictx[30088:45132,:]
prevrf = trainpredictx[45132:60176,:]
prevlb = trainpredictx[60176:75220,:]
prevb = trainpredictx[75220:90264,:]
prevrb = trainpredictx[90264:,:]
prev = prev.reshape(-1,2)
prevx = np.array(prev[:,0]).reshape(-1,5)
prevy = np.array(prev[:,1]).reshape(-1,5)
